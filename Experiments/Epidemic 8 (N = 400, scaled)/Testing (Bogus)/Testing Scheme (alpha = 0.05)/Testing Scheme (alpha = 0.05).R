...
setwd("C:/Users/Work/Documents/git/COVID19UK/Experiments/Epidemic 8 (N = 400, scaled)/Testing (Bogus)/Testing Scheme (alpha = 0.05)/")
...
obsParam <- c(alpha = .05, pi = 1, psi = 0)
obsWindow <- c(0, 1)
...
for(i in 1:6){
  ...
  legend("topright", legend = c("prop. rejcted:", sum(is.infinite(MCsamples))/length(MCsamples) ))

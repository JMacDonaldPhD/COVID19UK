#' Psuedo-Marginal MCMC for Hospital Admissions model

# fix everything but alpha. Then do folded unit RWM on alpha.
# alpha, rho.
# alpha, rho, gamma
# alpha, rho, gamma, betas

# COMMENT: alpha will is assumed to be independent of betas, rho and gamma. So proposals of alpha will be independent.

# Add adaptive step

PM_MCMCCOVIDLancs.ADAPTIVE = function(totalHospAdmissions, fixedParam, alpha0, alphaPrior, kernelParam, simulator, noSims,
  ...
  while(logPostCurr == -Inf){
    ...
    if(runParallel){
      ...
      sims <- lapply(1:noSims, function(X) simulator(param = c(fixedParam, alphaCurr), kernelParam)$dailyNoCases)
    }
...
PM_MCMCCOVIDLancs = function(totalHospAdmissions, fixedParam, alpha0, alphaPrior, kernelParam, simulator, lambda, noSims, noIts,

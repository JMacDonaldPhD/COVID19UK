...
Testing_Conditioned_NonCentered_HouseholdSIR <- function(pop, sampleData, startTime = 0, endTime){
  ...
  obs_index <- which(colSums(sampleData$firstStageTests) > 0)
  ...
  dailyProg <- function(day, StateX, Hstate, beta_G, beta_H, gamma, U, V, P_bias){
    ...
    for(i in obs_index){
      ...
      # x_it
      no_obs <- sampleData$firstStageTests[day, i] # Need at least no_obs infections
      ...
      # \sum_{s > t} x_is
      #futureKnownInfections <- sum(sampleData$firstStageTests[futureDays, i])


      # Conditions on number of infections
      # x_it < y_it < S_i(t-1) - \sum_{s > t} x_is
      #possibleNoInfections <- no_obs:(Hstate[i, 1])
      ...
      c <- pbinom(c(no_obs - 1, possibleNoInfections), size = Hstate[i, 1], prob = infProb[i])
      ...
      U_cond <- runif(1, min = bounds[1], max = bounds[2])
      #noInf[i] <- ((no_obs - 1):Hstate[i, 1])[which(U[i] < c)[1]]
      noInf[i] <- c(no_obs - 1, possibleNoInfections)[which(U_cond < c)[1]]

...
\begin{align*}
\prod_{i = 1}^{H} \sum_{(SR)_{i+}, I_{i+}} &{{S_i + R_i}\choose{(SR)_{i+}}} \psi^{(SR)_{+i}} (1 - \psi)^{S_{i} + R_{i} - (SR)_{i+}} \\ &{{I_i - (S \to I)_i}\choose{I_{+i}}} \pi^{I_{+i}} (1 - \pi)^{I_{i} - (S \to I)_{i} - I_{+i}} \\  &\cdot 1\{(SR)_{+i} + I_{+i} = T^{(2)}_{i+}\}\hspace{10mm} \text{(Second Stage Testing)}
\end{align*}
...
beta_G.seq <- seq(0.001, 0.25, length = 200)
beta_H.seq <- seq(0.001, 0.25, length = 200)
